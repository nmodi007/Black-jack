    ...
    def play(self, players, tray):
        ...
        while players_queue:
            ...
            while player.hands:
                ...
                if len(hand) == 1:
                    hand.cards.extend(tray.deal()) # should happen after split.
                ...
                if player_has_natural_balckjack and dealer_has_natural_balckjack:
                    player.records.append(Record(hand, self._hand, Status.Push))
                    player.balance.append(player.balance[-1])
                    self.bank.append(self.bank[-1]) 
                    continue
                if player_has_natural_balckjack and not dealer_has_natural_balckjack:
                    player.records.append(Record(hand, self._hand, Status.Win))
                    player.balance.append(player.balance[-1] + bet * 1.5)
                    self.bank.append(self.bank[-1] - bet * 1.5) 
                    continue
                if not player_has_natural_balckjack and dealer_has_natural_balckjack:
                    player.records.append(Record(hand, self._hand, Status.Lose))
                    player.balance.append(player.balance[-1] - bet)
                ...
                while True:
                    ...
                    if player_decision == Decision.Hit:
                        ...
                        if hand_busted(hand):
                            player.records.append(Record(hand, self._hand, Status.Busted))
                            player.balance.append(player.balance[-1] - bet)
                            self.bank.append(self.bank[-1] + bet)
                            break
                        else:  # we have an ace
        ...
        for player in players:
            if player.hands or player.staying_hands:
                raise Exception('players hands and stayings hands must be empty when play is over.')
